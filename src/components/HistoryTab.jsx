            ...
            <div key={item.id} className={`p-4 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2 text-sm text-gray-500">
                  <span>{SUPPORTED_LANGUAGES[item.fromLang].flag} {SUPPORTED_LANGUAGES[item.fromLang].name}</span>
                  <span>→</span>
                  <span>{SUPPORTED_LANGUAGES[item.toLang].flag} {SUPPORTED_LANGUAGES[item.toLang].name}</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <p className="text-indigo-600">Output: {item.translatedText}</p>
